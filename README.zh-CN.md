...
**Glimpse 窗口里**用的是左侧边栏而不是顶部标签栏，所以按键不同：点边栏切视图，`↑` / `↓` 在待审列表内移动，`Tab` / `Shift+Tab` 在当前打开那组里逐个跳设置字段，`Esc` 关窗。
...
设置字段自带控件：可枚举字段是下拉框（点它，或聚焦后按 `Space`，弹出系统菜单），自由文本字段是可直接输入的输入框，被 `XPI_MEMO_*` 变量钉住的字段是禁用态。控件一上报改动就落盘，没有单独的保存步骤；语言字段会让窗口原地重绘。`立即整理一次` 会先要确认，且不写配置。待审与最近页的详情跟随面板语言——包括记忆类型名，所以中文窗口不会冒出 `Constraint`；最近页按事件类型读取各自的字段，而不是显示一整行「—」。
...
- `XPI_MEMO_SLEEP_MODEL` = `provider/model-id`，仅供专用 sleep 运行使用（仅环境变量，无同名配置键；其余 `sleepMode` 取值不会读取它）
