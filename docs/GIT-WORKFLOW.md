...
- 默认在 `main` 上直接**提交**，**不默认推送**：**不建分支**。推送、分支提交或发布流程都需要用户显式提出。
...
**默认不会出现**「建分支」「开 PR」「推送」：这三项只在用户显式提出时才进入候选。

## 4. 分支策略（默认不建分支）

**默认不建分支，也不推送。** 直接在 `main` 上提交是本仓库的常规回路；推送是独立动作，需用户显式提出。
...
- `git fetch` 是只读的，可以默认执行；`git push` 不是默认动作，只在用户显式要求时执行。
...
这是本仓库的常规路径，不需要任何额外授权，也不需要建分支、开 PR 或推送：
...
完整检查点见 §3；分支、PR 与推送只在用户显式提出时进入流程。

### 11.2 需要同步或推送远端（推送需用户显式提出）

1. `git fetch origin`
2. 检查是否有本地未提交改动
3. 如果只是落后，`git pull --rebase`
4. 如果冲突，先停
5. 用户明确要求推送时：确认本地仅领先、无分叉后再 `git push`
