...
本文件只描述 `xpi-memo` 的仓库级 GitHub 约束。
...
- `xpi-memo` 目前按**阶段一**处理：单人快速迭代优先。
- 当前仓库不启用 `release-please`。
- **阶段一明确允许 main 直推**：提交保持小粒度 Conventional Commits、可回滚；发布按"bump 版本 → 打 tag → GitHub Release"直接在 main 上完成（v0.1.0–v0.4.0 均循此例）。
- 无远端、无历史提交的新脚手架允许在当前主分支创建首次本地提交；这是直推保护分支规则之外的初始化例外。
- 如果以后切到阶段二，再把流程改成"分支 + PR + 人工合并"为主，本条直推授权随之失效。
