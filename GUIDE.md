...
| `offlineExtractionModel` | `XPI_MEMO_OFFLINE_EXTRACTION_MODEL` | `session-model` | Model for the default extraction runner: `session-model` reuses the chat model, otherwise `provider/model-id` (bare `model-id` also works). An unknown id falls back to the session model. Read-only in the console — edit the config file or the env var |
| `autoAdmit` | `XPI_MEMO_AUTO_ADMIT` | `true` | Config-file switch for auto-admission: a verified `project_gene` candidate stores directly unless this is `false`. An explicitly set env var overrides the config file, and `XPI_MEMO_AUTO_VERIFY=false|0` still wins over both; while admission is off, a pass is recorded as `shadow-verified` and the candidate stays pending |
