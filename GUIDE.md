...
**Tool-verified auto-admission.** `project_gene` and `project_constraint` candidates are verified against the working tree with ripgrep before entering the pending queue: a fixed-string match of the candidate's verifiable line (500 ms timeout, test directories and lockfiles excluded). A passing match upgrades the evidence from `l0-conclusion` to `verified-repository-fact` and stores the candidate directly into its routed project bank — no confirmation dialog. It records `candidate_auto_verified` + `candidate_confirmed` in L0 and a `tool-verified` audit entry with the matched file path and line. A failed match, a missing `rg`, a timeout, and every other kind (e.g. `project_decision`) queue the candidate unchanged. Set `XPI_MEMO_AUTO_VERIFY=false` to queue everything for manual review.
...
| — | `XPI_MEMO_AUTO_VERIFY` | `true` | Env-only. `false` disables tool-verified auto-admission: `project_gene`/`project_constraint` candidates stop being verified against the working tree and all candidates queue for manual Store/Later/Reject |
