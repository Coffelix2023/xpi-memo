...
- **Project Domain Memory** — `.pi/DNA.yaml` holds two hand-editable, agent-written domains (`art` visual taste, `write` writing habits); fail-closed schema, user-authored entries are immutable to the agent, delivered by whole-domain injection (never recall), trusted projects only — human taste in the file, engineering in the bank
...
The **Settings** page groups its fields the way the TUI does, but as a top tab strip: one tab per group (Retrieval, Storage, Pipeline, Mental models, Admission, Decision, Display, Privacy). Click a tab — or put focus on the strip and use `←` / `→` — to bring its fields forward; only the selected panel scrolls, so the strip never scrolls out of reach. Whichever row is highlighted is explained in the detail pane at the bottom.
